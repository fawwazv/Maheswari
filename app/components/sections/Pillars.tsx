      ...
      <div className="max-w-7xl mx-auto px-gutter-mobile lg:px-gutter">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={item.title} className="bg-surface-subtle p-8 rounded-xl shadow-sm hover:shadow-md transition-all flex flex-col justify-between group border border-transparent hover:border-border-refined">
              <div>
                <div className="w-12 h-12 rounded-lg bg-surface-container flex items-center justify-center text-primary-container mb-6 group-hover:bg-primary-container group-hover:text-on-primary transition-colors">
                  <span className="material-symbols-outlined text-[24px]" aria-hidden>{item.icon}</span>
                </div>
              ...
              <div className="mt-6 pt-2 text-secondary font-label-sm text-label-sm font-semibold tracking-wider uppercase">{item.label}</div>
            </div>
          ))}
        </div>
      </div>
